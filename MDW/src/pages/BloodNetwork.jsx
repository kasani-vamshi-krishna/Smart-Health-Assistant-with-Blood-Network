...
import { FaSearch, FaPhoneAlt, FaMapMarkerAlt, FaTint, FaUsers, FaHandHoldingHeart, FaHospital, FaLock, FaShieldAlt } from 'react-icons/fa';

const BloodNetwork = () => {
    const { user } = useAuth();
    const isHospital = user?.role === 'hospital' && user?.verified;
    ...
    // Gate: non-hospital users see stats + locked notice only
    if (!isHospital) {
        return (
            <PageLayout title="Blood Donor Network">
                <div className="bg-gradient-to-r from-red-50 to-pink-50 border-2 border-red-200 rounded-2xl p-8 mb-6">
                    <div className="flex items-start gap-4">
                        <div className="w-14 h-14 bg-red-500 text-white rounded-2xl flex items-center justify-center flex-shrink-0">
                            <FaShieldAlt className="text-2xl" />
                        </div>
                        <div>
                            <h2 className="text-xl font-bold text-gray-800">Donor directory is hospital-only</h2>
                            <p className="text-gray-600 mt-1">
                                To protect donor phone numbers and personal data from misuse,
                                only <strong>verified hospitals and clinics</strong> can search donor contact details.
                            </p>
                            <ul className="mt-3 text-sm text-gray-600 space-y-1 list-disc list-inside">
                                <li>Hospitals register with a verified license number to gain access.</li>
                                <li>As a donor, your details are only shown to verified organisations.</li>
                                <li>You can enable / disable blood donation anytime in <Link to="/profile" className="text-indigo-600 font-semibold hover:underline">Profile</Link>.</li>
                            </ul>
                            <div className="mt-4 flex flex-wrap gap-2">
                                <Link to="/profile"
                                    className="inline-flex items-center gap-2 px-4 py-2 bg-red-500 text-white font-semibold rounded-lg hover:bg-red-600">
                                    <FaTint /> Manage my donor status
                                </Link>
                                {user?.role !== 'hospital' && (
                                    <span className="inline-flex items-center gap-2 px-4 py-2 bg-white text-gray-700 font-semibold rounded-lg border">
                                        <FaLock /> Registered as an individual
                                    </span>
                                )}
                            </div>
                        </div>
                ...
                {stats && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="bg-gradient-to-br from-indigo-500 to-indigo-600 text-white rounded-2xl p-6 shadow-lg">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-indigo-100 text-sm font-medium">Community Members</p>
                        ...
                        <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 text-white rounded-2xl p-6 shadow-lg">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-emerald-100 text-sm font-medium">Verified Hospitals</p>
                                    <p className="text-4xl font-extrabold mt-1">{stats.total_hospitals}</p>
                                </div>
                                <FaHospital className="text-5xl text-emerald-300 opacity-80" />
                            </div>
                        </div>
                    </div>
                )}
    ...
    return (
        <PageLayout title="Blood Donor Network">
            <div className="space-y-6">
                <div className="bg-gradient-to-r from-red-500 to-red-600 text-white rounded-2xl p-5 flex items-center gap-3">
                    <FaHospital className="text-3xl" />
                    <div>
                        <p className="font-bold">{user?.hospital_name}</p>
                        <p className="text-xs text-red-100">Verified · License: {user?.license_number}</p>
                    </div>
                </div>
                    ...
                    <form onSubmit={handleSearch} className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <select value={searchForm.blood_type}
                                    onChange={(e) => setSearchForm(prev => ({ ...prev, blood_type: e.target.value }))}
                                    className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-red-500 outline-none bg-white text-lg font-semibold text-red-600">
                            ...
                            <div className="flex items-end">
                                <button type="submit" disabled={loading}
                                    className="w-full py-3 bg-gradient-to-r from-red-500 to-red-600 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50 flex items-center justify-center gap-2">
                                    {loading ? (
                        ...
                        {results.total === 0 ? (
                            ...
                            <div className="space-y-3">
                                {results.donors.map((donor) => {
                                    const conds = donor.donor_health_conditions || {};
                                    const condList = Object.entries(conds)
                                    ...
                                    return (
                                        <div key={donor.id}
                                            ...
                                            <div className="flex items-start gap-4">
                                                <div className="w-14 h-14 bg-gradient-to-br from-red-500 to-red-600 rounded-2xl flex items-center justify-center text-white font-extrabold text-lg shadow-md flex-shrink-0">
                                                    {donor.blood_type}
                                                </div>
                                                <div>
                                                    ...
                                                    {condList.length > 0 && (
                                                        <div className="mt-2 flex flex-wrap gap-1">
                                                            {condList.map(c => (
                                                                <span key={c} className="text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full">
                                                                    ⚠ {c}
                                                                </span>
                                                            ))}
                                                        </div>
                                                    ...
                                                    {conds.other && (
                                                        <p className="mt-1 text-xs text-gray-500 italic">Note: {conds.other}</p>
                                                    )}
                                                </div>
                                            </div>
